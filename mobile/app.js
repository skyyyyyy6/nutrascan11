...
axios.interceptors.response.use(
  (response) => {
    ...
    return console.log("cek dikit", response.data);
  },
  (error) => {
...
const App = () => {
  ...
  const BASE_URL = 'https://b7db-103-84-5-55.ngrok-free.app';   // API URL
    ...
    if (cameraRef.current) {
      try {
        ...
        const resizedPhoto = await ImageManipulator.manipulateAsync(
          photo.uri,
          [{ resize: { width: 160, height: 160 } }],  // Aggressive resize to 160x160
          { compress: 0.2, format: ImageManipulator.SaveFormat.JPEG }  // Strong compression (reduce file size)
        );
  ...
  const recognizeFood = async (uri) => {
    ...
    try {
      ...
      console.log("Sending image to APIssss...");
  
      const response = async () => {
        try {
          await axios.post(apiUrl, { image: base64Image })
            .then((res) => {
              console.log(res);
            });
        } catch (error) {
